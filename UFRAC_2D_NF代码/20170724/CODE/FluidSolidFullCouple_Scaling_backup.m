function  FluidSolidFullCouple_Scaling_backup(CurT,dt,isGrow)
...
global DD_global PresF_global AllEle_global nAllEle_global ConnList_global;
global CpF_global XfF_global nFracture Fractures;
clear isMechActive0;
...
for i = 1 : nAct
    ...
    P_pre(i) = PresF_global(IndexInv(i))*1.02*1e6;
...
for i = 1 : nAct
    %alpha = theta + 90;
    sinalp = AllEle_global(IndexInv(i),6);  %cosbet(i);
    cosalp = -AllEle_global(IndexInv(i),5); %-sinbet(i);
...
Xf = Xf0;
for ii = 1 : nAct
    if abs(AllEle_global(IndexInv(ii),10) - 3)<1e-3
        if isMechActive(ii) > 0.1
            e0(ii) = e0(ii) + calcNfWidth(P_pre(ii));
        else
            % Closed Fracture
            xx= AllEle_global(IndexInv(ii),8)+epsd*AllEle_global(IndexInv(ii),5);
            yy = AllEle_global(IndexInv(ii),9)-epsd*AllEle_global(IndexInv(ii),6);
            [Sxxi,Syyi,Sxyi,~,~] = CalcPointStress_C(Mat,DD_pre,xx,yy,AllEle_global,ConnList_global);
            Sxxi = Sxxi + Mat.Sxx;
            Syyi = Syyi + Mat.Syy;
            Sxyi = Sxyi + Mat.Sxy;
            cosb = AllEle_global(IndexInv(ii),6);
            sinb = AllEle_global(IndexInv(ii),5);
...
for ii = nAct:-1:1
    if abs(TipStates(IndexInv(ii))) > 1e-8
        labor = ConnList_global(IndexInv(ii),3);
        if labor < 0.1
            labor = ConnList_global(IndexInv(ii),4);
        end
        labor = Index(labor);
    ...
    while dis > tol || isRegroup > 0.1
        ...
        if EleType == 1
            if iIter < 1.1 && isGrow > 0.1
                [extraBC,SS]  = BuildCoefMatix_Constant(Mat,nFracture,Fractures,nAllEle_global,AllEle_global,ConnList_global,1);
            else
                [extraBC,SS]  =BuildCoefMatix_Constant(Mat,nFracture,Fractures,nAllEle_global,AllEle_global,ConnList_global,0);
            end
        else
            if iIter < 1.1 && isGrow > 0.1
                [extraBC,SS]  = BuildCoefMatix_H2(Mat,nFracture,Fractures,nAllEle_global,AllEle_global,ConnList_global,1);
            else
                if iIter < 1.1
                    [extraBC,SS]  = BuildCoefMatix_H2(Mat,nFracture,Fractures,nAllEle_global,AllEle_global,ConnList_global,0);
                end
            end
        end
        ...
        for ii = 1 : nAct
            EleL(ii) = AllEle_global(IndexInv(ii),7);
            if isMechActive(ii) < 0.1
                if isMechActive(ii) > -0.1
                    %  totally closed
                    BC(ii) = 0;
        ...
        for i = 1: nAct
            ...
            % Flow Flux Term
            for j = 1 : nConn
                if ConnList_global(IndexInv(i),j+2) > -0.1
                    Conj =  ConnList_global(IndexInv(i),j+2);
                    if Index(Conj) < 0.1
                        continue;
                    end
            ...
            Jac(nAct+i,i) = Jac(nAct+i,i) - dDdp*AllEle_global(IndexInv(i),7)*h*b/dt*unitQ*Dchara;
            %Newly Modified,Accumulation Terms
            dVdt = (V(i)*b-e0(i)*b_init*h*AllEle_global(IndexInv(i),7))/dt*unitQ;
            if abs(dVdt) < 1e-16
                dVdt = 0;
            end
        ...
        for ii = 1 : nwell
            for jj = 1 :well{ii}.Sch(Schindex(ii)).nPf
                ...
                num = IndexInv(well{ii}.Perfindex(iele,:));
                RHS(num(1)+nAct) = RHS(num(1)+nAct) + well{ii}.Sch(Schindex(ii)).PfQsl(jj)/1e3;
                RHS(num(2)+nAct) = RHS(num(2)+nAct) + well{ii}.Sch(Schindex(ii)).PfQsl(jj)/1e3;
            end
        end
