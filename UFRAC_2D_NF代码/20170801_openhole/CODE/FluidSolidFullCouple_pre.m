function  [dt,DD_new,e_new,Pre_new,AllElenew] =FluidSolidFullCouple_pre(EleType,Mat,P_pre,DD_pre,dt,nAllEle,AllEle,ConnList,nFracture,Fractures,nwell,well,isGrow)
...
global InitialAperture InitialAperture2
global TipStates;
restart = 1;
...
for ii = 1 : nAct
    if abs(AllEle(IndexInv(ii),10) - 3)<1e-3
        ...
        shearS_sign =  -(Sxxi - Syyi)*sinb*cosb+ Sxyi*(cosb^2 - sinb^2);
        shearS = abs(shearS_sign)*1e6;
        if P_pre(ii) < sigmaN(ii)*tolNorm
            % Closed NF
            isMechActive(ii) = 0;
            [e0(ii),~] = calcNfWidth(P_pre(ii));
            %sliding
            iparent = ConnList(IndexInv(ii),1);
            fang = Fractures{iparent}.Fang;
            coh = Fractures{iparent}.Coh*1e6;
            miu = tand(fang);
            if shearS > miu*(sigmaN(ii)-P_pre(ii)) + coh || abs(AllEle(IndexInv(ii),11)) > 0.1
                % Sliding
                isMechActive(ii) = -1;
                if abs(AllEle(IndexInv(ii),11)) > 0.1
                    ShearBC(ii) = AllEle(IndexInv(ii),11)*(miu*(sigmaN(ii)-P_pre(ii)));
                else
                    ShearBC(ii) = -sign(shearS_sign)*(miu*(sigmaN(ii)-P_pre(ii)));
                    AllEle(IndexInv(ii),11) = -sign(shearS_sign);
                end
            end
        else
...
while restart > 0.1
    ...
    while dis > tol || isRegroup > 0.1
        ...
        for i = 1: nAct
            ...
            for j = 1 : nConn
                if ConnList(IndexInv(i),j+2) > -0.1
                    ...
                    if P(i) >= P(Index(Conj))
                        ...
                        Jac(nAct+i,nAct+i) = Jac(nAct+i,nAct+i) + Tij * ((b*revmiu_difp + revmiu*b_difp)*(P(Index(Conj)) - P(i)) - revmiu*b)*unitC + T_difPi * (revmiu*b)*(P(Index(Conj)) - P(i));
                        Jac(nAct+i,nAct+Index(Conj)) = Jac(nAct+i,nAct+Index(Conj)) +Tij * (revmiu*b)*unitC + T_difPj * (revmiu*b)*(P(Index(Conj)) - P(i));
                    else
                        b = Calc_Bw(P(Index(Conj)),1);
                        b_difp = Calc_Bw(P(Index(Conj)),2);
                        revmiu =Calc_miu(P(Index(Conj)),1);
                        revmiu_difp =Calc_miu(P(Index(Conj)),2);
                        Jac(nAct+i,nAct+i) = Jac(nAct+i,nAct+i) +  Tij * (-revmiu*b)*unitC + T_difPi * (revmiu*b)*(P(Index(Conj)) - P(i));
                        Jac(nAct+i,nAct+Index(Conj)) = Jac(nAct+i,nAct+Index(Conj)) + Tij * (revmiu*b+(b*revmiu_difp + revmiu*b_difp)*(P(Index(Conj)) - P(i)))*unitC + T_difPj * (revmiu*b)*(P(Index(Conj)) - P(i));
                    end
                    if isnan(Jac(nAct+i,nAct+i))
                        keyboard;
                    end
                    Jac(nAct+i,i) = Jac(nAct+i,i) +T_difDi * (-1)* (revmiu*b)*(P(Index(Conj)) - P(i));
                    Jac(nAct+i,Index(Conj)) = Jac(nAct+i,Index(Conj)) +T_difDj * (-1)*(revmiu*b)*(P(Index(Conj)) - P(i));
            ...
            Jac(nAct+i,nAct+i) = Jac(nAct+i,nAct+i) - V(i)*b_difp/dt*unitQ - (dwdp*b*h*EleL(i))/dt*unitQ;
            if isnan(Jac(nAct+i,nAct+i))
                keyboard;
            end
            % For Opening Derivative
            Jac(nAct+i,i) = Jac(nAct+i,i) - dDdp*AllEle(IndexInv(i),7)*h*b/dt*unitQ;
            ...
            % Inj
            for nww = 1 :nwell
                if IndexInv(i) == well{nww}.index
                    RHS(i+nAct) = RHS(i+nAct) + well{nww}.q*unitQ;
                end
            end
            %Leak
        ...
        DD = [Ds;Dn];
        %Update States
        tempStatus = isMechActive;
        for ii = 1 : nAct
            if abs(AllEle(IndexInv(ii),10) - 3)<1e-3
                % Closed Fracture
                xx= AllEle(IndexInv(ii),8)+epsd*AllEle(IndexInv(ii),5);
                yy = AllEle(IndexInv(ii),9)-epsd*AllEle(IndexInv(ii),6);
                [Sxxi,Syyi,Sxyi,~,~] = CalcPointStress_C(Mat,DD,xx,yy,AllEle,ConnList);
                Sxxi = Sxxi + Mat.Sxx;
                Syyi = Syyi + Mat.Syy;
                Sxyi = Sxyi + Mat.Sxy;
                cosb = AllEle(IndexInv(ii),6);
                ...
                % shearS_sign =  -(Sxxi - Syyi)*sinb*cosb+ Sxyi*(cosb^2 - sinb^2);
                % shearS = abs(shearS_sign)*1e6;
                % Only change no-sliding elements
                if isMechActive(ii) > -0.1
                    if P(ii) > sigmaN(ii)*tolNorm
                        isMechActive(ii) = 1;
                    else
                        isMechActive(ii) = 0;
                    end
        ...
        updateStatus = isMechActive;
        isRegroup = 0;
        if norm(updateStatus - tempStatus) > 1e-2
            isRegroup = 1;
        end
        %  [isRegroup,AllEle] = RegroupNF(Fractures,nAllEle,AllEle,ConnList,DD,P);
...
%                 coh = Fractures{iparent}.Coh*1e6;
%                 miu = tand(fang);
%                 if shearS > miu*(sigmaN(ii)-P(ii)) + coh || abs(AllEle(IndexInv(ii),11)) > 0.1
%                     % Sliding
%                     % if isMechActive(ii) > -0.9
%                     isMechActive(ii) = -1;
%                     if abs(AllEle(IndexInv(ii),11)) > 0.1
%                         ShearBC(ii) = AllEle(IndexInv(ii),11)*(miu*(sigmaN(ii)-P(ii)));
%                     else
%                         ShearBC(ii) = -sign(shearS_sign)*(miu*(sigmaN(ii)-P(ii)));
%                         AllEle(IndexInv(ii),11) = -sign(shearS_sign);
%                     end
%                     
%                     % end
