...
global FILEPATH_main  Kindex RecordK;
...
fidDn  = fopen([Path,'centerDn.in'],'a+');
fidDs  = fopen([Path,'centerDs.in'],'a+');
fprintf(fidDs,'%f %f\t\n',CurT, DD_global(IndexInv(1)));
fprintf(fidDn,'%f %f\t\n',CurT, -DD_global(IndexInv(1)+MaxEle));
fclose(fidDn);
fclose(fidDs);
for welli = 1 : nwell
