function [isMechActive,ShearBC,sigmaN,e0,tau] = CheckStates(nAct, AllEle, epsd,DD_pre,P_pre,Mat, fric, Maxtau)
...
for i = 1 : nAct
    ...
    if  AllEle(i,10) > 1.1
        ...
        e0(i) = ei;        
        if sigmaN(i) < P_pre(i)*1.05
            isMechActive(i) = 1;
        end
        if sigmaN(i) >= P_pre(i)*1.05
            isMechActive(i) =0;
        end
        if  abs(tau(i)) <= fric*(sigmaN(i) - P_pre(i))*0.95 && isMechActive(i) < 0.1
            isMechActive(i) = -2;
        end
