...
fric = tand(Mat.fric);
[isMechActive0,ShearBC,sigmaN,e0,tau] = CheckStates(nAct, AllEle, epsd,DD_pre,P_pre,Mat, fric, Maxtau);
...
while restart > 0.1 || iIterT > 0.1
   % isMechActive = isMechActive0;
    ...
    [~,~,sigmaN,~] = CheckStates(nAct, AllEle, epsd,DD_pre,P_pre,Mat, fric, Maxtau);
    ...
    while dis > tol || isRegroup > 0.1
        ...
        if abs(det(A110)) < 1e-6 ||  isinf(det(A110)) > 0.1
            f = 1;
        end
        invA11 = A110\tempI;
        R1 = BC(1:nAct);
...
%                         keyboard;
        ...
        for i = 1 : nAct
            if AllEle_global(IndexInv(i),10) > 1.1
                %                 if isMechActive(i) < -1.1
                %                     Ds(i) = 0;
                %                 end
                %                % if sign(Ds(i)-Ds0(i))*ShearBC(i) <- 1e-6 && isMechActive(i) < 0.1
                %                 if sign(Ds(i))*sign(Ds0i(i)) < -1e-6 && isMechActive(i) < 0.1
                %                     isMechActive(i) = -2;
                %                     dis = 1e6;
                %                     Ds(i) = 0;
                %                     Dn(i) = 0;
    ...
    %% Check Time step
    temp = dt;
    if restart < 0.1 && 0% min(isMechActive) > 0.1
