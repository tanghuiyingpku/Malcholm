function [leftT2,rightT2,isAdjust,dt] = AdjustPropSpeed(leftT,rightT,dt0,CritK,KI1,KI2)
...
for i = 1 : nActTip
    ...
    if AllEle_global(num,10) > 1.1 &&  ConnList_global(num,3)*ConnList_global(num,4) > -1e-5
        %NF
%         if AllEle_global(num,10) > 2.2  &&  ConnList_global(num,3)*ConnList_global(num,4) < -1e-5
%             maxKv(i) = CritK(i);
%         else
